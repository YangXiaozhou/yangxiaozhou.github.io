...
1. sda $wsd$ sda
2. da $ \hat{\mathbf{\Sigma}} = \mathbf{U}\mathbf{D}\mathbf{U}^{T} \,. $
3. inline with double dollar $$\hat{\mathbf{\Sigma}} = \mathbf{U}\mathbf{D}\mathbf{U}^{T} \,. $$
4. Something

1. Perform eigen-decompostion $s$ on the pooled covariance matrix: $ \hat{\mathbf{\Sigma}} = \mathbf{U}\mathbf{D}\mathbf{U}^{T} \,. $
2. Sphere the data:
$$
