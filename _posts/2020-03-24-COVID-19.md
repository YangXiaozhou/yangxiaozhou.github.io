...
tags: 
---
**Any conclusion drawn from the data should be viewed with caution due to the dynamic nature of a pandemic and the adundant sources of bias associated with reporting.** I update the current COVID-19 situation in the US, Europe, and Asia, tracking both the outbreak and any emerging signal of containment. The graphs in this blog are best viewed on a Laptop browser. Latest update: {{ site.time | date: '%B %d, %Y' }}. 
...
### Percentage Change
Look out for the 7-day moving average (MA) of the percentage change in confirmed cases reported. It is important to see both the current percentage change and its trend. To easily classify the situation, we can use the following scale[^percentage]:
