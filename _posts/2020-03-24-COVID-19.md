...
## 1. Prognosis
- **Asia**
    + Singapore sees a steady increase in confirmed cases, mainly in foreign worker dormitory clusters. However, if we look at the [progression of confirmed cases](#Case progression) in Singapore, it's an almost perfect example of what "flatten the curve" looks like. For the most part, the cases double every ten days, whereas cases in some of the worst-hit countries double every one to three days.   
    + Japan also has a relatively flat curve. However, there are legitimate concerns that Japan has been under-testing its population. For example, Japan has conducted 437 tests [per one million population](#https://www.worldometers.info/coronavirus/). In Singapore, that number is 11,110. 
- **United States**
    - The increase of new cases is slowing down [(12%)](#Percentage Change). The US has conducted more than 1.3 million tests over the past ten days, or on average, 130,000 per day. The [number of tests pending results](#US Testing Numbers) also dropped from 61,980 to around 16,000. 
    - The case fatality rate (CFR) continues to rise, likely due to an increasingly overwhelmed healthcare system.
- **Europe**
    + The containment efforts are taking effect as the increase of cases has significantly slowed down, with Italy, the slowest, at 4%, and Spain, the fastest, at 6%. 
    + There are signs of recovery as well. The worst-hit countries in Europe, Italy, and Spain, are seeing a [stabalization of death rates](#case fatality rate). 
    ...
    + Note there are caveats to reporting naive CFR, which I [explain below](#cfr bias). 
...
- Germany and Switzerland, on the other hand, fare well in this regard and manage to record comparatively low CFRs. Austria, too, has managed one of the lowerest CFRs among European nations. (Austria, Germany, and a large part of Switzerland are all German-speaking.🤔)
